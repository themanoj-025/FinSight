...
class FinanceAgent:
    def __init__(
        ...
        if not self._anthropic and self.api_key:
            ...
            except (RuntimeError, ValueError, OSError) as exc:  # noqa: BLE001 — degrade offline
    ...
    def _llm_answer(
        ...
        for _ in range(max_turns):
            ...
            except (RuntimeError, ValueError, OSError) as exc:  # noqa: BLE001 — graceful degradation
                latency_ms = (time.perf_counter() - t0) * 1000
    ...
    def _execute_tool(self, name: str, args: dict[str, Any]) -> tuple[str, bool, str, float]:
        ...
        except (RuntimeError, ValueError, OSError) as exc:  # noqa: BLE001 — tool execution boundary
            ms = (time.perf_counter() - t0) * 1000
